...
import { Music, Send, CheckCircle, List, Trash2, Martini } from 'lucide-react';
...
export default function MusicRequestApp() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (JSON.stringify(refreshedData) !== JSON.stringify(lastData)) {
  ...
  if (view === 'admin' && isAdmin) {
    return (
      <div className="min-h-screen bg-forajido-premium p-4">
          ...
          {requests.length === 0 ? (
            ...
            <div className="space-y-3">
              {requests.map((r) => (
                <div
                  key={r.id}
                  className="bg-white rounded-xl shadow-lg p-5 flex justify-between items-start hover:shadow-xl transition-shadow"
                >
                  <div>
    ...
    <div className="min-h-screen bg-forajido-premium flex items-center justify-center p-4">
      <div className="bg-[#141414]/90 backdrop-blur-md border border-[#C6A664]/30 rounded-3xl shadow-xl p-10 w-full max-w-md text-center text-[#F5F5F5]">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-[#FFD166] mb-2">
            Solicita tu Canción
          </h1>

          <p className="text-white">¿Qué te gustaría escuchar?</p>
        </div>
        ...
        {submitted ? (
          <div className="text-center py-8">
            ...
            <p className="text-white">El DJ revisará tu petición</p>
          </div>
        ) : (
          <div className="space-y-5">
            <div>
